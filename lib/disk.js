...
Disk.prototype = {
  ...
  readMBR: function( callback ) {
    ...
    this.device.readBlocks( 0, 1, function( error, buffer ) {
      ...
      try { self.mbr = Disk.MBR.parse( buffer ) }
  ...
  readGPT: function( callback ) {
    ...
    if( this.mbr == null ) {
      return done( new Error( 'Missing Master Boot Record' ) )
    }
    
    var efiPart = this.getEFIPart()
    if( efiPart == null ) {
      return done( new Error( 'Missing EFI System Partition entry' ) )
    }
    ...
    this.device.readBlocks(
      ...
      function( error, buffer ) {
        ...
        try { self.gpt = Disk.GPT.parse( buffer ) }
        catch( e ) { return done( e ) }
        ...
        self.device.readBlocks(
          ...
          function( error, buffer ) {
            ...
            try { self.gpt.parseTable( buffer ) }
            catch( e ) { return done( e ) }
